...
fn has_exact(tokens: &TokenRegistry, class_name: &str, property: &str, value: &str) -> bool {
...
fn has_contains(tokens: &TokenRegistry, class_name: &str, property: &str, needle: &str) -> bool {
    let parsed = parse_class_name(class_name).unwrap();
    let rule = resolve_rule(&parsed, tokens).unwrap();
    rule.declarations
        .iter()
