...
pub fn test_commit_message(root: &Path, failures: &mut Vec<String>, warnings: &mut Vec<String>) {
    ...
    if subject.is_empty() {
        failures.push("HEAD commit message subject is empty".to_string());
        return;
    }
    ...
    if subject.ends_with('.') {
        failures.push("HEAD commit subject must not end with a period".to_string());
    }
    ...
    if let Some((commit_type, description)) = split_subject(subject) {
        ...
        if description.is_empty() {
            failures.push("HEAD commit subject must include a description after `: `".to_string());
        } else if description
            .chars()
            .next()
